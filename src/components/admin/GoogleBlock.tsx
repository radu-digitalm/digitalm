...
export function GoogleBlock({ prospect, enabled }: { prospect: ProspectWithGoogle; enabled: boolean }) {
  ...
  return (
    <details className="card p-5" data-testid="google-block" open={open} onToggle={(e) => setOpen(e.currentTarget.open)}>
      <summary className="cursor-pointer text-[20px] text-fg-heading">{fill(GOOGLE_TEXT.summary, { status: googleStatusWord(status, true) })}</summary>
      <div className="mt-4 space-y-3">
        <div className="flex flex-wrap items-center gap-2" data-testid="google-status">
          ...
          {reasonWord ? <span className="text-[15px] text-fg-muted">{reasonWord}</span> : null}
        </div>
        ...
        {signals ? (
          <div data-testid="google-signals" className="space-y-1">
            <p className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[16px] text-fg">
              {googleSignalWords(signals).map((w) => (
                <span key={w}>{w}</span>
              ))}
              <GoogleAttribution />
            </p>
            {dataLine ? <p className="text-[15px] text-fg-muted">{dataLine}</p> : null}
          </div>
        ) : null}
        ...
        {p.googlePlaceId ? (
          <details data-testid="google-panel" onToggle={(e) => setPanelOpen(e.currentTarget.open)}>
            <summary className="cursor-pointer text-[16px] text-fg-muted">{GOOGLE_TEXT.showListing}</summary>
        ...
        {usage ? <p className="text-[15px] text-fg-muted">{fill(GOOGLE_TEXT.usageLine, { used: usage.checks.used, cap: usage.checks.cap })}</p> : null}
        ...
        {candidates && candidates.length > 0 ? (
                ...
                return (
                  <li key={c.placeId} className="flex items-start justify-between gap-3">
                    <span className="min-w-0">
                      ...
                      {data ? <span className="block text-[15px] text-fg-muted">{data}</span> : null}
                    </span>
          ...
          <p className="text-[16px] text-fg-muted">{GOOGLE_TEXT.noCandidates}</p>
        ) : null}
      </div>
    </details>
  );
