      ...
      <div className="relative min-h-0 flex-1">
        <div className="h-full space-y-4 overflow-y-auto px-4 pb-10 pt-4">
          <header>
            ...
            <p className="mt-1 text-[16px] text-fg-muted">{GOOGLE_TEXT.googleOnlyHint}</p>
          </header>
      ...
      <div className="space-y-2 border-t border-line px-4 py-3">
        {pin.saved ? (
          <div className="flex flex-wrap items-center gap-2">
            <Link href={`/admin/prospects/${pin.saved.prospectId}`} className="btn-primary inline-flex items-center px-4 py-2 text-[16px]" data-testid="card-open-prospect">
              {CARD_TEXT.openProspect}
            </Link>
            <span className="text-[15px] text-fg-muted">{fill(CARD_TEXT.alreadySaved, { reference: pin.saved.reference })}</span>
          </div>
        ) : (
          <>
            <div className="flex flex-wrap items-center gap-2">
            ...
            <p className="text-[15px] text-fg-muted">{pin.hidden ? CARD_TEXT.hidden : GOOGLE_TEXT.needsWebsite}</p>
          </>
        )}
      </div>
    </div>
