 ...
 *  finder-ux integration: replace with `searchSummary(id)` from lib/discover once it exports it. */
function foundBy(searchId: number | null): { area: string; trade: string; createdAt: string } | null {
  if (!searchId) return null;
  const s = listSearches(100).find((x) => x.id === searchId);
  if (!s) return null;
  return { area: s.areaLabel, trade: s.categoryKey.startsWith("custom:") ? s.categoryKey.slice(7).replace(/_/g, " ") : tradeLabel(s.categoryKey) ?? s.categoryKey, createdAt: s.createdAt };
}
