...
> **Finder UX v2 (12 Sep 2026).** What follows is the v1 finder (bounding rectangles, areas ≤ 1.5 deg², a 40 s budget) and is kept for the adapters, categories, dedupe, save and views. It was redesigned after Radu's review: `docs/finder-ux-spec.md` is the current contract and wins wherever the two differ — polygon areas through Overpass area selectors (Ariège no longer includes Andorra or Spain), regions and countries accepted behind a cap gate with one-click sub-searches, a resumable background runner with live progress, a Leaflet map with pins and a business card, plain words in the UI (no "OSM", "FR reg.", "partial", "diffusion"). The v2 API is summarised in §16. One exception to the §0 "no new runtime dependency" rule: `leaflet@1.9.4` (CSS bundled locally, OpenStreetMap tiles with the "© OpenStreetMap contributors" attribution). **Google (12 Sep 2026):** `docs/finder-google-spec.md` adds Google Places as a further discovery source merged with these adapters, Google's place suggestions in the Area box and a Google map in place of Leaflet, all behind `GOOGLE_PLACES=on` (off = unchanged); it wins over this section wherever they differ. API additions in §17; key set-up and the live acceptance run in `docs/finder-google-live-checklist.md`.
...
> **Google-listing check (12 Sep 2026).** The `google_listing` check of §7.2 (10 points, `not_measured` until now) is measured by `docs/finder-google-spec.md` §4.5–§4.6 when `GOOGLE_PLACES=on`: a saved prospect is matched to a Google place (Text Search, name + 150 m / postcode, once every 30 days) and its listing is checked with Place Details (Enterprise fields, under `GOOGLE_PLACES_MONTHLY_CAP`); a listing found means a Business Profile exists, and open status, website on the listing, hours, review count and photo count score 4 + 1 + 2 + 1 + 1 + 1 (pass ≥ 9). Google's API exposes no "claimed" field, so the check speaks of a listing *found* / *looks maintained*, never *claimed* or *verified*; only a manual *Not this listing* yields `not_found` / `no-gbp`. Off = the check stays unmeasured as before. `docs/finder-google-spec.md` wins over this section wherever they differ; API summary in §17, live acceptance in `docs/finder-google-live-checklist.md`.
...
Google Places, a Google map and a real Google-listing check were added on 12 Sep 2026: **`docs/finder-google-spec.md` wins over §6–§7 wherever they differ**; with `GOOGLE_PLACES=off` nothing changes. API additions (spec §4.8), all under the existing auth/CSRF rules:

- `POST /api/admin/find` body adds `suggestion?: { placeId }` (a Google place id from the Area box; any other key ignored); the `202` response carries `queryArea` (the derived query, e.g. "Ariège, France"). New errors: `400 bad_suggestion` (malformed id), `400 suggestion_unresolved` (empty `area` and Google could not resolve the id), `429 rate_limited` (60 suggestion picks / 10 min per IP).
- `GET /api/admin/find?id=&after=&v=&pins=` — `SearchResultV2` adds `googlePins?: GooglePin[]` (present only when the Google phase ran; on a full read, or when `pins` is absent or behind `progress.google.pinsVersion`), `progress.google?: GoogleProgress`, `progress.stage` may be `"google"`, rows may carry `googlePlaceId` and `onGoogle`.
- `POST /api/admin/find/dismiss { searchId, key, undo? }` — `key` may be `google:<placeId>` (`400 bad_key` when malformed, `404` when the pin is not in the search).
- `POST /api/admin/prospects { url, name?, country, googlePlaceId? }` — stores the place id as a manual match (`400 bad_place_id`); the `409 already_saved` answer adds `placeAttached?: boolean`.
- `POST /api/admin/prospects/[id]/google { action: "check" | "find" | "confirm" | "reject" | "clear", placeId? }` → `{ ok, prospect, signals, usage }` / `{ ok, candidates, usage }`; errors `501 google_off`, `429 google_monthly_cap { pool }`, `502 google_unavailable`, `502 google_refused`, `429 rate_limited` (30 / 10 min per IP). `GET /api/admin/prospects/[id]` adds `googleCheckedAt`, `googleMatch`, `googleSignals`.
- Types under `// @@finder-google:types` in `src/lib/crm/types.ts`; schema `prospects.google_checked_at`, `prospects.google_match`; `api_usage` providers `google_search`, `google_details_enterprise`, `google_details_other` (Pacific day keys) and `google_check_day`. Env: the `# ---- Google` block of `.env.example`.
