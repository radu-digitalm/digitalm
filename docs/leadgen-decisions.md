...
- **Sending:** from the existing site mailbox (`CONTACT_FORM_FROM`, display name "Radu — Digital M" via `OUTREACH_FROM_NAME`), `OUTREACH_FROM` env override kept. Daily cap starts at 10.
...
- **Cron:** approved — one entry in hermes's crontab at 08:00 for the Telegram digest (`/home/hermes/workspace/scripts/digitalm-digest.js`, read-only, idempotent; see the build spec §5 for the wrapper).
