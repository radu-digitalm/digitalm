...
<!-- @@finder-google:backend -->
**Google in the finder and the audit (`docs/finder-google-spec.md`, backend).** Behind `GOOGLE_PLACES=on` + `GOOGLE_PLACES_KEY` (nothing changes while off): `src/lib/discover/google.ts` is the one Places API (New) client — three monthly pools in `api_usage` (`google_search`, `google_details_enterprise`, `google_details_other`) counted on the Pacific calendar day, reserved before any socket, one of three frozen field masks per request (`googleRequests.ts`: Text Search Pro, Details Enterprise, Details Essentials — anything else is refused as `mask_tier`), the key in a header and never in a log. Nothing from Google is cached and only the place id, lat/lng inside the 24 h search cache, derived booleans/counts and attribution provider names are ever stored. Area suggestions: `POST /api/admin/find { suggestion: { placeId } }` → one Details Essentials call → Nominatim with a hint (`resolveArea(query, pick, hint)` auto-picks among ambiguous candidates). Discovery (`GOOGLE_PLACES_DISCOVERY=on` only): a Google phase after the register (`googleDiscover.ts` — our polygon clips the tiles, the inside filter reads address components only, `googleMatch.ts` matches by name + 150 m / postcode); matched rows carry `googlePlaceId` / `onGoogle`, the rest become anonymous `googlePins` (dismissable as `google:<placeId>`). Listing check (`googleCheck.ts` + the pure `googleCheckRules.ts`): a saved prospect without a place is matched once every 30 days (Text Search Pro), a place is checked with Details Enterprise and the derived signals — open, website on the listing, hours, review and photo counts — score the `google_listing` check (4 + 1 + 2 + 1 + 1 + 1; pass ≥ 9); only a manual "Not this listing" yields `not_found`. Prospects without a website get a Google-only audit at priority 8 under `GOOGLE_CHECK_DAILY_CAP`. `POST /api/admin/prospects/[id]/google { action: check | find | confirm | reject | clear }`. Env: the `# ---- Google` block of `.env.example`. Tests: `googleRequests`, `googleTiles`, `googleInside`, `googleMatch`, `googleCheckRules`, `googleStatic` (the terms as code rules), fixtures under `src/lib/discover/fixtures/google/` (synthetic).

<!-- @@finder-google:frontend -->
**Google in the finder (frontend, `docs/finder-google-spec.md`).** Everything sits behind `GOOGLE_PLACES=on`; with it off the Leaflet finder, the plain Area box, the hidden Google block and the unmeasured *Google listing* check are exactly as before. With it on and the two build-time browser values set (`NEXT_PUBLIC_GOOGLE_MAPS_KEY`, `NEXT_PUBLIC_GOOGLE_MAP_ID` — rebuild after changing them), `/admin/find` draws **Google Maps** (`GoogleFindMap.tsx`: one Map instance per page open, reused across searches; the same dashed outline, pin colours, unit dots and progress bar; Advanced Markers clustered by `@googlemaps/markerclusterer`; `© OpenStreetMap contributors` on the map for our own pins; on any load failure the Leaflet map returns under one line of explanation), the Area box offers **Google's place suggestions** (`GoogleAreaInput.tsx`: the Places UI Kit autocomplete element created imperatively, a pick posts `suggestion: { placeId }` and the box then shows the area the search resolved to), and places only Google knows appear as **blue pins** with an *Only on Google* chip and a card that is Google's own listing panel plus *Add by its website* / *Not this one* (`GoogleOnlyCard.tsx`, `GoogleListingPanel.tsx`). Rows Google also knows get *Also on Google · View on Google Maps* and a compact Google panel in the business card. The prospect page's **Google listing** section (`GoogleBlock.tsx`, after the audit block, collapsed) shows the status in words, the derived signals as words with the Google Maps logo and *Data: …* when Google named a provider, Google's full panel, and Check again / Match manually / Not this listing / Clear; while it is open the mini map is hidden (`html[data-google-open]`) and no signal word appears anywhere else — `AuditBlock` shows status words only (`googleWords.ts`, tested). Attribution is the official logo (`public/brand/google-maps-logo*.svg`, `GoogleAttribution.tsx`); the text form only in the list-header count. Wording in `GOOGLE_TEXT` (`wording.ts`, forbidden tokens now include *claimed*, *verified*, *Google My Business*, *GMB*); the loader and place-id helpers in `googleMaps.ts`; the admin CSP entry in `next.config.mjs` exists only when the browser key is set at build time. Acceptance: items G1, G8, G9 of `scripts/e2e/finder.cjs` (skipped with *skipped — Google off* when the finder renders Leaflet).

<!-- @@finder-google:integration -->
**Google mode — switches, keys, terms (merged 12 Sep 2026).** Two switches and two key pairs, all in the `# ---- Google` block of `.env.example`. `GOOGLE_PLACES=on` + `GOOGLE_PLACES_KEY` (server key, API-restricted to *Places API (New)*, read only in `src/lib/discover/google.ts`, never sent to a browser) turn on the listing check in every audit, the prospect page's Google section and the Area-box suggestions; `NEXT_PUBLIC_GOOGLE_MAPS_KEY` + `NEXT_PUBLIC_GOOGLE_MAP_ID` (browser key restricted to *Maps JavaScript API* + *Places UI Kit* and to the referrers `https://digitalm.eu/*` and `https://d3v.digitalm.eu/*`; both are build-time values — rebuild after changing them) switch the finder map from Leaflet to Google Maps; `GOOGLE_PLACES_DISCOVERY=on` additionally asks Google Places during a search (decision D1, default off). Off = today's behaviour, untouched. On without a server key = the Leaflet map, no Google call anywhere, a note in each search and a *Google key* card on Today. Spend stays inside Google's free tier by construction: the three monthly pools (`GOOGLE_PLACES_MONTHLY_CAP` 900 listing checks · `GOOGLE_SEARCH_MONTHLY_CAP` 4,500 searches · `GOOGLE_DETAILS_MONTHLY_CAP` 4,500 suggestion look-ups) are 90 % of the free allowances, reserved in `api_usage` before any socket and counted on the Pacific day; one search spends at most `GOOGLE_SEARCH_MAX_REQUESTS` (30) and discovery stops 500 short of the search pool; Google-only audits are capped by `GOOGLE_CHECK_DAILY_CAP` (30/day); the per-day quotas set in the Cloud Console (spec §6) are the hard stop for the browser key. Google's terms as enforced in code (spec §3, `googleStatic.test.ts`): Google results are drawn only on the Google map — the Leaflet map never receives `googlePins`; only the place id is kept for good, everything else is derived booleans/counts with `fetchedAt`, re-fetched or dropped after 30 days, and nothing from Google enters `api_cache` or a log; Pro-tier field masks on searches, Enterprise fields only in the listing check of a saved prospect; the Google Maps logo (`GoogleAttribution`) accompanies every Google-derived word shown outside a map and Google's own panels carry their own attribution; no scraping, no pre-fetch; no *claimed* / *verified* wording — the API has no such field, so the check reads *listing found · looks maintained*. Console set-up, the env lines and the live acceptance run: `docs/finder-google-live-checklist.md`.
